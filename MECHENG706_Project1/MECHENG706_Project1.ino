...
void CoordUpdate(float IRSensor) {
...
void MiddleLogic() {
  ...
  if (((FR_IR_Data[0] + BR_IR_Data[0]) / 2) > ((FL_IR_Data[0] + BL_IR_Data[0]) / 2)) {
    ...
    isLeft = 0; //The wall is on the left, but we want to drive right.
...
void MiddleStrafe(bool Left, float iAngle) {
  // The bool Left variable determines if the robot wants to go left. The wall is on the opposite side.
...
void FindCorner()
{
  ...
  if (BR_IR[0] < 20) {
    ...
    ultraDist = HC_SR04_range();
    if (ultraDist > 130) {
      StartedOnTheRight = true;
      return;
    }
    else {
      StartedOnTheRight = false;
      TurnByAngle(-90);
      return;
    }
  }
